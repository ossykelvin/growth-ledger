...
export default function Invoices() {
  ...
  const fetchInvoices = async () => {
    if (!user) return;
    setLoading(true);
    const { data } = await supabase.from("tbl_invoices").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
    setInvoices(data || []);
    setLoading(false);
  };
    ...
    <div className="space-y-8">
      ...
      <div className="glass-card overflow-hidden">
        {loading ? (
          ...
          <div className="py-12 text-center text-muted-foreground">No invoices yet. Click "New Invoice" to create one.</div>
        ) : (
